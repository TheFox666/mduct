# mduct Management Layer Plan (Plan 2 of 2)

> Execution notes for the same-session implementer. TDD red-green per task,
> commit per green cycle. Spec: `docs/specs/2026-07-19-mduct-design.md`.
...
- `mduct add <name> -- <command…>` (+ `--env K=V…`, `--note`, `--url` variant),
  `mduct remove/enable/disable <name>`. Daemon hot-reload makes changes live.
...
- `mduct import --from-claude [--source <path>] [name…]`: no names → list
...
- Sections: (a) overlap: servers in Claude sources ALSO served by mduct →
  print removal commands per source; (b) dead servers: configured in mduct but
...
- `MDUCT_REGISTRY` base URL override (tests use local fixture server).
...
- `mduct search <q>` prints `ref\tdescription`. `mduct add <ref>` fetches manifest,
  prompts for required env vars only when TTY (else requires `--env`).
- Bare `mduct add` (TTY only): numbered list picker over search results +
...
- `mduct hook install claude [--config-dir <dir>] [--settings <file>]`:
  1. writes `~/.config/mduct/hooks/session-start.sh` → emits `mduct index` +
     overlap-nudge (doctor-lite) to stdout (SessionStart stdout = context).
  2. writes `pre-tool-use.sh` → stdin JSON, if tool_name matches `mcp__(<mduct-served>)__`
     → JSON deny decision with `mduct call <server> <tool>` replacement hint.
...
- `mduct daemon --install`: write systemd user unit (After=default.target,
  ExecStart=<abs mduct> daemon, Restart=on-failure) + `systemctl --user enable --now`.
