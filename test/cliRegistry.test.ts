...
const env = {
  ...
  MDUCT_REGISTRY: `http://localhost:${fixture.port}`,
};
...
test("search prints ref<TAB>description", async () => {
  const r = await mduct("search", "gitlab");
...
test("add by registry ref installs http remote under --as name", async () => {
  const r = await mduct("add", "com.gitlab/mcp", "--as", "gitlab");
  expect(r.code).toBe(0);
