...
const env = { ...process.env, MCPMUX_CONFIG: join(dir, "servers.jsonc"), MCPMUX_HOME: home };
...
test("import by name writes the server into mux config", async () => {
  const r = await mux("import", "linear");
  expect(r.code).toBe(0);
