...
test("logs <server> does not leak another server's lines (#19/N6)", async () => {
  ...
  writeFileSync(process.env.MDUCT_CONFIG, `{"servers":{${line("lab")},${line("gitlab")}}}`);
  const d = await startDaemon();
  stop = d.stop;
  await request(process.env.MDUCT_SOCKET, "call", { server: "gitlab", tool: "echo", args: { text: "x" } });
  const labLogs = (await request(process.env.MDUCT_SOCKET, "logs", { server: "lab" })) as string[];
  ...
  const gitlabLogs = (await request(process.env.MDUCT_SOCKET, "logs", { server: "gitlab" })) as string[];
