...
export async function cmdSearch(query: string | undefined): Promise<number> {
  if (!query) { console.error("usage: mduct search <query>"); return 1; }
...
/** Registry install: `mduct add <ref-with-slash> [--as name] [--replace]`. */
async function addFromRegistry(ref: string, as: string | undefined, replace: boolean): Promise<number> {
  ...
  if (!hit) {
    ...
    console.error(`"${ref}" not found in registry${near ? ` — did you mean: ${near}` : ""} — try: mduct search <query>`);
    return 1;
  }
  ...
  console.log(`try: mduct tools ${name}`);
  return 0;
}

/**
 * `mduct add <name> [--url u | -- cmd…] [--env K=V…] [--note n] [--replace]` — manual, AX path.
 * `mduct add <ref-with-slash> [--as name]` — registry install (refs look like com.gitlab/mcp).
 */
...
function cmdAddManual(argv: string[]): number {
  ...
  if (isTool) {
    if (!name || command.length === 0) {
      console.error("usage: mduct add <name> --tool [--check <cmd>] [--setup <cmd>] [--env K=V] [--note text] -- <command…>");
      return 1;
    }
    const tool: ToolCfg = {
    ...
    console.log(`added tool: ${name} — try: mduct run ${name}`);
    return 0;
  }

  if (!name || (!url && command.length === 0)) {
    console.error("usage: mduct add <name> --url <url> | mduct add <name> -- <command…>  [--env K=V] [--note text] [--replace]\n       mduct add <name> --tool -- <command…>   (register a CLI tool)");
    return 1;
  }
  ...
  console.log(`added: ${name} — try: mduct tools ${name}`);
  return 0;
}

export function cmdRemove(name: string | undefined): number {
  if (!name) { console.error("usage: mduct remove <server>"); return 1; }
  removeServer(name);
...
export function cmdSetDisabled(name: string | undefined, disabled: boolean): number {
  if (!name) { console.error(`usage: mduct ${disabled ? "disable" : "enable"} <server>`); return 1; }
