...
/** `mux secret set|list|rm`. `set` reads the value from stdin (pipe) or a hidden TTY prompt. */
export async function cmdSecret(argv: string[]): Promise<number> {
  ...
  if (sub === "rm") {
    if (!argv[1]) { console.error("usage: mux secret rm <NAME>"); return 1; }
    rmSecret(argv[1]);
  ...
  if (sub === "set") {
    const name = argv[1];
    if (!name) { console.error("usage: mux secret set <NAME>   (value from stdin or hidden prompt)"); return 1; }
    let value: string;
  ...
  console.error("usage: mux secret set <NAME> | mux secret list | mux secret rm <NAME>");
  return 1;
}
